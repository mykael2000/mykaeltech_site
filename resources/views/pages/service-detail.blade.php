<x-app-layout>
    ...
    <section class="bg-glow relative overflow-hidden">
        <div class="mx-auto max-w-4xl px-4 py-20 sm:px-6 lg:px-8" data-reveal>
            <a href="{{ route('services') }}" class="text-sm font-semibold text-brand-300 hover:text-brand-200">← All services</a>
    ...
    <section class="mx-auto max-w-4xl px-4 pb-24 sm:px-6 lg:px-8 space-y-16">
        <!-- Description -->
        <div>
        ...
        @if ($service->starting_price)
            <div class="rounded-2xl border border-brand-400/20 bg-gradient-to-br from-brand-600/10 to-violet-600/10 p-8" data-reveal>
                <div class="flex items-center gap-3 text-sm font-semibold uppercase tracking-wider text-brand-300">
                    <span>💰</span> Starting price
                </div>
        ...
        <!-- CTA -->
        <div class="rounded-2xl border border-brand-400/20 bg-brand-500/5 p-8 text-center" data-reveal>
            ...
            <div>
                ...
                <div class="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3" data-reveal>
                    @foreach ($relatedProjects as $project)
                        <a href="{{ route('portfolio.show', $project->slug) }}" class="card-hover glass rounded-2xl p-5 hover-zoom">
